...
class OilDataset (Dataset):
    ...
    def __getitem__(self, index):
        ...
        for i in range(index, (index + self.groupdaynumb)):
            ...
            #rowday = copy.deepcopy(self.sheet.row_values(i))
            ...
            if self.datatype == 'KP':
                ...
                #rowday[0][Oilstart:Oillen + 1].replace('\'', ' ')
                #for m in range(0,len(rowday)):
                groupData.append(rowday[0][Oilstart:Oillen+1].replace('\t',',\''))
            else:
        ...
        if  self.datatype == 'KP' and len(groupData) == self.groupdaynumb * kpstepnumb:
            ...
            maxvalue = np.array(np.max(inputdata))
            minvalue = np.array(np.min(inputdata))
        elif  self.datatype == 'Oil' and len(groupData) == self.groupdaynumb * 1:
            labeldata = np.array(groupData[-self.predlen * 1:][0][-1]).astype('float32')
            inputdata = np.array(groupData[:-self.predlen * 1]).astype('float32').reshape(-1, 1)
            maxvalue = np.array(np.max(inputdata))
            minvalue = np.array(np.min(inputdata))
        else:
